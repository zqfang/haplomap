...
BlockSummary::BlockSummary(const char *chrnm, int num, int start, int size,
                           int chrbeg, int chrend, const char *pat) : blockIdx(num), blockStart(start), blockSize(size),
...
void BlockSummary::updateGeneIsInteresting()
{
    ...
    for (std::map<std::string, std::string>::iterator giit = this->geneIsCodingMap.begin(); giit != this->geneIsCodingMap.end(); giit++)
    {
        ...
        int flag = this->isNumber(giit->second) ? std::stoi(giit->second) : this->updateCodonScore(giit->second);

        if (this->geneIsInteresting.count(giit->first) > 0) // .find() != .end()
        {
            // if gene is exists in this haploblock, take max of flag
            this->geneIsInteresting[giit->first] = std::max(flag, this->geneIsInteresting[giit->first]);
        }
        else
        {
...
GhmapWriter::GhmapWriter(char *outputFileName, char *datasetName, bool categorical, float pvalCutoff, const char *datasetFormat) : _dataset_name(datasetName), _dataset_format(datasetFormat), _outFileName(outputFileName), isCategorical(categorical), pvalueCutoff(pvalCutoff)
{
    ...
    os << "##" << _dataset_name << "\t"
       << "FORMAT=" << _dataset_format << std::endl;
