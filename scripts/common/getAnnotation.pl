...
my $outputManuallyExcludedGenes = 0; ### used only for the bad_gene_info file

#my $inputMasterFolder = "/home/mzheng/NGS/combined_results/20180101/";
my $inputMasterFolder = "/u07/combined_results/20190301/";
#my $inputMasterFolder = "/home/mzheng/mzheng-data/projects/RNA_seq/genentech/201805/DNAseq/interested/";
#my $inputMasterFolder = "/home/mzheng/mzheng-data/temp/test_chr/";
#my $inputMasterFolder = "/u06/PeltzLabData/SNP_data/combined/20180101/";

my $inputFileHeader = $inputMasterFolder."chr";
#my @interestedStrains = (0..6);
my @interestedStrains = (0..52);
my $outputFile = $inputMasterFolder."SNP_codon_"."v$version.txt";
my $outputCodonChangeFile = $inputMasterFolder."codon_change_"."v$version.txt";
my $outputGeneInfoFile = $inputMasterFolder."gene_info_"."v$version.txt";
my $outputBadGeneInfoFile = $inputMasterFolder."bad_gene_info_"."v$version.txt";

my $inputGeneFileHeader = "/home/mzheng/mzheng-data/ensembl_data/EMBL/mouse/B38/v$version/Mus_musculus.GRCm38.$version".".chromosome.";
my $exclusionListFile = "/home/mzheng/mzheng-data/ensembl_data/EMBL/mouse/B38/v$version/gene_exclusion_list.txt";
...
my ($f, $i, $j, $k, $key, $line, $chr, $inputFileName, $count, $tempPos, $tempRef, $tempLocalID, $tempSSID, $tempAccNum, $isSNP, $isGood, $insertPos, $tempAlternative, $thisTag);
...
for($i = 0; $i < scalar(@interestedChr); $i++)
#foreach $i (22)
#$i = 22;
{
    my ($seqio, $seqobj);
    $currentFile = $inputGeneFileHeader.$interestedChr[$i].".dat";
	...
	### only look at chr 1-19 and X, Y, MT.
...
#	    print "primary tag: ", $feat_object->primary_tag, "\n";
	    my %allTags = ();
	    my @allTagValues = $feat_object->get_all_tags();
	    foreach $thisTag (@allTagValues)
	    {
		$allTags{$thisTag} = 1;
	    }
...
#		    print "gene $currentGene, file $currentFile, is in reverse order\n";
		...
		if($feat_object->has_tag("standard_name"))
		{
		    @temp = $feat_object->get_tag_values("standard_name");
		    $currentTranscript = $temp[0];
		}else
		{
		    ...
		    {
			die "transcript name for this gene $ensGene not found properly\n";
		    }
		}
		...
		for($j = 0; $j < $#startLoc; $j++)
		{
			...
			{
			    if($tempSNPCodon[$index - $indexSG] eq "unspecified")
			    {
...
print "start the final step of annotate intergenic SNPs\n";
...
{
    if($outputManuallyExcludedGenes == 1 || $notGoodGenes{$i} ne "excluded_manually")
    {
...
sub getSNPIndex
{
    ...
    die "SNPPosArray in getSNPIndex is empty\n" if scalar(@{$SNPPosArray}) == 0;
    my ($indexS, $indexE);
