...
void writeSortedPattern(ostream &os, char *pattern, vector<int>& strOrderVec) {
  ...
  for (int strIdx = 0; strIdx < numStrains; strIdx++) {
    ...
    if ('?' != eqclass) {
      if ('?' == sortedEqMap[eqclass]) { // class hasn't been mapped yet.
		   ...
		   vector<BlockSummary *>& blocks, float cutoff, bool filterCoding)
{
  ...
  // write them out.
  for(vector<GeneSummary *>::iterator git = genes.begin();
      git != genes.end();
      git++) {
...
int scoreChanges(string str){
  ...
  while(true) {
    ...
    if(AACLASSES[aa1] != AACLASSES[aa2]) {
      count++;
      int X = 'X' - 'A';
      if(aa1==X || aa2 == X) count+=2; // Higher weight if stop codon
    }
  }
  return count;
}
...
{  
  ...
  while ((numtoks = rdr.getLine()) >= 0) {
    ...
    if (eqclass != "0") equalStrains.push_back(i);
    i++;
  }
}
