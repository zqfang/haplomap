...
//     pattern = dupPattern(snpInfo.pattern);
  ...
  // destructor
  ~SNPInfo() { free(alleles); if (!frozen) { free(pattern); } }
...
public:
  ...
  void init() { pattern = (char *) malloc(numStrains); memset(pattern, '?', numStrains); };
...
int combinePatternNoQs(char *combined, int blockstart, int blocksize, int haploLimit) {
  ...
  //cout << endl;
  if(numAssigned==numStrains) return haploLimit;
  return 0;
}
...
void ffbBinSearch(HaploBlock &bestBlock, int blockstart, int haploLimit, int minSize, int maxSize = -1)
{
  ...
  while (true) {
    ...
    if (numHaplo == 0) {	// combinePatterns exceeded limit
...
bool fmbBinSearch(HaploBlock *pHaploBlock, int blockstart, int minSize, int blocksize, int haploSize, int chrIdx, int firstLoc )
{
  ...
  if (traceFBB) {
    cout << "fmbBinSearch:" << " blockstart: " << blockstart
...
void findCompoundBlocks(int haploLimit)
{
  ...
  for (int blockstart = 0; blockstart < blockstop; blockstart++) {
    ...
    if (useFastFBB) {
      ...
      if (0 == blockstart || snpVec[blockstart-1]->pattern != snpVec[blockstart]->pattern
	  || (snpVec[blockstart]->position - snpVec[blockstart-1]->position) > tooLong
	  || (snpVec[blockstart-1]->chrIdx != snpVec[blockstart]->chrIdx)) {
...
void chooseBlocks()
{
  ...
  while ((begit = sortedCompoundHaploBlocks.begin()) != sortedCompoundHaploBlocks.end()) {
	...
	if (j > 0) {
	  ...
	  if (traceChooseBlocks) {
	    cout << "Truncating block to size " << j << ":" << endl << *phb << endl;
	  }
	  if (useFastFBB) {
...
void writeBlock(ostream& hs, int blkIdx, int haploLimit)
{
  ...
  for(size_t i = 0; i < strainDispVec.size(); i++) {
    ...
    hs << "<TH>";
    if (strainDisp.numSNPsDefined < blocksize) { hs << "<FONT COLOR=gray>"; }
    hs << strainAbbrevs.eltOf(strainDisp.strIdx) ;
    if (strainDisp.numSNPsDefined < blocksize) { hs << "</FONT>"; }
    hs << "</TH>";
  }
  hs << "</TR>" << endl;
