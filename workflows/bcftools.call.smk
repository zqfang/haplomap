    ...
    run:
        with open(output[0], w) as out:
            for i in params.chrom:
                if str(i) == "1": continue 
                outline = f"chr{i}.bed\tchr{i}.bin\chr{i}.fam\n"
                out.write(outline)

rule mergebed:
    input: 
        merge = "PLINK/merglist.txt",
        beds = expand("PLINK/chr{i}.bed", i=CHROMSOME),
        bims = expand("PLINK/chr{i}.bim", i=CHROMSOME),
        fams = expand("PLINK/chr{i}.fam", i=CHROMSOME),
    output:
         "PLINK/inbred.bed",
         "PLINK/inbred.bim",
         "PLINK/inbred.fam"
    shell:
        "plink --bfile chr1 --merge-list {input.merge} --make-bed --out PLINK/inbred"
...
rule rel_distance_matrix:
    input:
        ...
        "PLINK/inbred.fam"
    output:
        "PLINK/inbred_grm.rel",
        "PLINK/inbred_grm.rel.id",
    threads: 12
    shell:
        "  plink --bfile PLINK/inbred " # need .bed, .bim, .fam
        "--make-rel square "
        "--out inbred_grm "
        "--threads {threads}"
...
rule rel_dist_ghmap:
    input:
        rel = "PLINK/inbred_grm.rel",
        names = "PLINK/inbred_grm.rel.id",
    output:
        rel = "PLINK/inbred_grm_ghmap.rel"
    run:
        samples = []
